...
export const getAppId = (transition: any) => {
    let rootPageData: any = {};
    transition({
        type: "PAGE_GET_STATE",
...
async function getUserContextFromDB(tenantInfo: any, appId: any) {
...
export async function getUserContext(appId: string | number, templateKey: string) { // once we get userContext from DB will store that value in localStorage
...
export function templateValueContainsUserKeyWord(userTemplateValue: string) {
...
export async function getUpdatedFilter(filter: any, queryParams: any, transition: any) {
