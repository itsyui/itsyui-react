...
import { DataLoaderFactory, ICommandManager, IDataSourceLake, ISchemaLoader } from "@itsy-ui/core";
...
export function doGridLoad(event: any) {
	return async (_, dispatch: any, transition: any) => {
		...
		dispatch({
			...
			typeId: event.typeId,
			rowSelectionMode: event.rowSelectionMode === undefined || event.rowSelectionMode === null ? 1 : event.rowSelectionMode,
			canTriggerGridSelectedRows: event.canTriggerGridSelectedRows === undefined || event.canTriggerGridSelectedRows === null ? true : event.canTriggerGridSelectedRows,
			context: event.context ? event.context : {},
			pageInfo: event.maxItems ? { maxItems: event.maxItems } : { maxItems: 10 },
			pageContext: event.pageContext ? event.pageContext : {}
		});
		if (event.typeId) {
			dispatch(onFetchDataServer(event.typeId, event.gridSchemaId, event.customDatasource, event.renderCustomCell, filterText, gridSchema, event.selectedRecords));
		} else {
			transition({
...
function getRow(row: any, columns: any, type: string): IGridDataRecord | IGridCellValue {
	const data: any | IGridCellValue = {};
	objectEntries(row).forEach((key) => {
		const findColumnType = columns.find(t => t.name === key[0] && t.type === "widget"); // Find type === widget
...
export function onFetchDataServer(
	typeId: string,
	gridSchemaId?: string,
	customDatasource?: IDataSourceLake, renderCustomCell?: any, filterValue?: any, gridSchema?: {}, selectedRecords?: any, incrementalAdd?: boolean, searchValue: any = null, isRefresh = false, sortingInfo: any = null): any {
	...
	return async (getState, dispatch: any, transition: any) => {
		...
		const datasource = customDatasource !== undefined && customDatasource !== null ? customDatasource : dataLoader.getLoader<IDataSourceLake>("datasource");
		const schemaLoaderFactory = WidgetsFactory.instance.services["DataLoaderFactory"] as DataLoaderFactory;
		const schemaLoader = schemaLoaderFactory.getLoader<ISchemaLoader>(GridActions.Loader);
		...
		if (gridSchema) {
			...
			typeData = await schemaLoader.getControlSchema({ typeId: typeId, gridSchemaId: gridSchemaId });
		}
		try {
			const parameters = typeData.viewAttributes ? Object.assign({}, typeData.viewAttributes) : {};
			...
			if (state.viewAttributes && state.viewAttributes.hasOwnProperty("context")) {
				parameters["context"] = state.viewAttributes["context"];
			}

			//Computing default filter
			parameters.defaultFilter = parameters.defaultFilter && Object.keys(parameters.defaultFilter).length > 0 ? parameters.defaultFilter : {};

			if (state.viewAttributes && state.viewAttributes.defaultFilter && Object.keys(state.viewAttributes.defaultFilter).length > 0) {
				parameters.defaultFilter = { ...parameters.defaultFilter, ...state.viewAttributes.defaultFilter };
			}

			if (filterValue && Object.keys(filterValue).length > 0) {
				parameters.defaultFilter = { ...parameters.defaultFilter, ...filterValue };
			}

			parameters.filter = searchValue && Object.keys(searchValue).length > 0 ? { ...parameters.defaultFilter, ...searchValue } : { ...parameters.defaultFilter };
			const orderBy = sortingInfo && Object.keys(sortingInfo).length > 1 ? `${sortingInfo.orderBy} ${sortingInfo.order}` : null;
			...
			for (const key in cData) {
				if (cData[key].propertyType === "widget") {
					...
					columns.push({
						...
						"label": getLocaleString(cData[key], "displayName"),
						"type": cData[key].propertyType,
						"isHidden": cData[key].isHidden === undefined ? defaultHiddenColumns.indexOf(cData[key].id) > -1 : cData[key].isHidden,
						"fieldSchema": cData[key],
					});
				}
			}
			...
			pageInfo.totalPage = Math.ceil(rowsData["totalRecordsCount"] / pageInfo.maxItems);

			const uniquePropertyId = typeData.uniquePropertyId ? typeData.uniquePropertyId : null; //uniquePropertyId is Mandatory for editing cell
			if (parameters.hasOwnProperty("customAction") && parameters.customAction.hasOwnProperty("actions") && parameters.customAction["actions"].length > 0) {
				parameters["customAction"]["actions"] = parameters["customAction"]["actions"].map(t => {
					return { ...t, "displayText": getLocaleString(t, "displayText") };
				});
			}
			dispatch(dataAvailable(typeId, gridSchemaId, result, uniquePropertyId, pageInfo, parameters, customDatasource, renderCustomCell, typeData, incrementalAdd, sortingInfo));
			if (selectedRecords !== undefined && selectedRecords !== null && selectedRecords.length > 0) {
				if (typeof selectedRecords[0] === "string") {
					const selectedRecordsArray = [];
					selectedRecords.forEach(ele => {
						const findObj = result["records"].find(t => (uniquePropertyId ? t[uniquePropertyId] : t["cmis:objectId"]) === ele);
...
function dataAvailable(typeId: string, gridSchemaId: string, data: any, uniquePropertyId: string, pageInfo: any, viewAttributes: any, customDataSource: any, renderCustomCell: any, typeData: {}, incrementalAdd: boolean, sortingInfo: any) {
	return {
...
export function doGridBeforeSelectedRows(event: any) {
	return async (getState: any, _dispatch: any, transition: any) => {
		...
		const uniquePropertyId = gridSchema.uniquePropertyId ? gridSchema.uniquePropertyId : null;
		if (typeof event.selectedRows[0] === "string") {
			event.selectedRecords.forEach(ele => {
				const findObj = data["records"].find(t => uniquePropertyId !== null ? t[uniquePropertyId] : t["cmis:objectId"] === ele);
				// tslint:disable-next-line: no-unused-expression
				findObj && selectedRecordsArray.push(findObj);
			});
		}
		...
		transition({
			type: canTriggerGridSelectedRows ? GridActions.State.GRID_SELECTED_ROWS : GridActions.State.GRID_SELECTED_ROWS_DONE,
...
export function doGridRefresh() {
	return async (getState: any, dispatch: any, transition: any) => {
		const { typeId, gridSchemaId, selectedRows, customDataSource, renderCustomCell, gridSchema, incrementalAdd, viewAttributes } = getState();
		const preAppliedFilter = viewAttributes.filter !== undefined ? viewAttributes.filter : {};
		dispatch(onFetchDataServer(typeId, gridSchemaId, customDataSource, renderCustomCell, null, gridSchema, selectedRows, incrementalAdd, preAppliedFilter, true));
		transition({
			type: GridActions.State.GRID_DONE,
		});
	};
...
export function onCustomButtonExecuteCommand(action: any, record: any, controlID: string, queryParams: any) {
	const dataLoader = WidgetsFactory.instance.services["DataLoaderFactory"] as DataLoaderFactory;
	...
	return async (getState: any, _dispatch: any, transition: any) => {
		...
		const contextParams: any = {
			...
			context: {
				...getCommandContext(action, { typeId, record, customDataSource, controlID }),
				pageContext
			},
		};
...
export function doInitRowSummary(typeId: string, viewAttributes: any, datasource: IDataSourceLake) {
	return async (_: any, dispatch: any, _transition: any) => {
		const columnsString = viewAttributes.totals.summaryColumns.map(t => t.name).join(",");
		const parameters = {
			filter: viewAttributes.filter !== undefined ? `${columnsString}, ${viewAttributes.filter.substring(2)}` : columnsString,
		};
		...
		if (viewAttributes.totals !== undefined) {
			...
			for (let i = 0; i < viewAttributes.totals.summaryColumns.length; i++) {
				...
				if (summaryColumn.type === "sum") {
					const colData = records.map(t => t[summaryColumn.name] !== undefined ? t[summaryColumn.name] : 0);
...
export function doGridFilter(params: any) {
	return async (getState: any, dispatch: any, transition: any) => {
		const { viewAttributes, typeId, gridSchemaId, customDataSource, renderCustomCell, incrementalAdd } = getState();
		let searchValue = {};
		if (params.searchValue) {
			if (typeof params.searchValue === "object") {
				searchValue = params.searchValue;
			} else if (Array.isArray(viewAttributes.filterColumns) && viewAttributes.filterColumns.length > 0) {
		...
		dispatch(onFetchDataServer(typeId, gridSchemaId, customDataSource, renderCustomCell, params.filter, null, null, incrementalAdd, searchValue, true));
		transition({
			type: "GRID_DONE",
		});
	};
...
export function doGridEdit(event: any) {
	return (getState, dispatch: any, transition: any) => {
		...
		if (editingRecords && editingRecords.length > 0) {
			const findObj = editingRecords.find(t => t[uniquePropertyId] === editingRecord[uniquePropertyId]);
			if (findObj) {
